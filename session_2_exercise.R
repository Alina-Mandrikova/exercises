...
#4. Assume `x` and `y` contain the amounts in dollars that 2000 different people possess. How much money do all people possess together?
  
x <- 1:1000
x <- seq(1, 1000, by = 1)
y <- 1000 - x
x+y
total_amount <- sum(x) + sum(y)
total_amount
...
n <- 1e4
scale <- 1.5e4
income <- round(rbeta(n = n, shape1 = 4, shape2 = 6) * scale, 2)
...
2. Use the income distribution `income` that you came up with to compute each person share of the total income of all people. 

total_income <- sum(income)
share <- (income / total_income) * 100
...
```{r}
income_s <- sort(income)  # Sort the income values in ascending order
group <- c("Lower 1%", "Lower 50%", "Top 10%", "Top 1%")  # Define the groups for income summaries
p <- c(.1, .5, .9, .99)  # Define the percentiles for income summaries

boundary <- round(income_s[p*n], 0)  # Compute the income boundaries for each group based on the given percentiles

# Calculate the means for each group by taking the average income within each range
low10_m <- mean(income_s[c(1:(.1*n))])  # Mean for the lower 1% group
low50_m <- mean(income_s[c(1:(.5*n))])  # Mean for the lower 50% group
top10_m <- mean(income_s[c((.9*n):n)])  # Mean for the top 10% group
top1_m <- mean(income_s[c((.99*n):n)])  # Mean for the top 1% group

means <- round(c(low10_m, low50_m, top10_m, top1_m), 0)  # Round the mean values to whole numbers

# Create a data frame to summarize the income distribution
income_summary <- data.frame(group, boundary, means)

income_summary  # Print 
...
# Creating a data frame with 5 variables and 5 rows
df <- data.frame(var1 = c(2, 2, 2, 2, 2),
                 var2 = c(5, 5, 5, 5, 5),
                 var3 = c(1, 2, 3, 5, 7),
                 var4 = c(5, 5, 8, 9, 7),
                 var5 = c(3, 3, 3, 3, 5))

# Compute the means of each variable and store in a vector
means <- colMeans(df)
# Compute the means of each variable in one line
means <- colMeans(data.frame(var1 = c(1, 2, 3, 4, 5),
                             var2 = c(6, 7, 8, 9, 10),
...
# Get an overview of the data set column names
names(diamonds)
...
#Obtain rows with ideal cut using matrix notation with which
ideal_cut_rows <- diamonds[which(diamonds$cut == "Ideal"), ]
# Obtain rows with ideal cut using filter function from dplyr
ideal_cut_rows <- filter(diamonds, cut == "Ideal")
# Obtain rows with ideal cut and select specific columns
ideal_cut_selected <- filter(diamonds, cut == "Ideal") %>%
